...
void skirstymas(list<data>& temp, list<data>& temp2, list<data>& temp3) {
	data laik;
	list<data>::iterator it;
	int n = temp.size();
	for (it = temp.begin(); it != temp.end(); it++) {
	...
	for (it = temp.begin(); it != temp.end(); it++) {
		if (it->rezultatas > 5.0) {
			laik2.vardas = it->vardas;
			laik2.pavarde = it->pavarde;
			laik2.rezultatas = it->rezultatas;
			laik2.rezultatasm = it->rezultatasm;
