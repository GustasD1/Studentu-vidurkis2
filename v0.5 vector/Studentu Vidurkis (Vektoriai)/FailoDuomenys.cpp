...
void FailoIsvedimas(vector<data>& temp, vector<data>& temp2) {
		...
		cout << "Duomenu sortinimas uztruko" << diff.count() << "s" << std::endl;
...
void skirstymas(vector<data>& temp, vector<data>& temp2, vector<data>& temp3) {
	data laik;
	int n = temp.size();
	for (int i = 0; i < n; i++) {
	...
	for (int i = 0; i < n; i++) {
		if (temp[i].rezultatas > 5.0) {
			laik2.vardas = "Vardas" + std::to_string(i + 1);
			laik2.pavarde = "Pavarde" + std::to_string(i + 1);
			laik2.rezultatas = temp[i].rezultatas;
			laik2.rezultatasm = temp[i].rezultatasm;
			temp2.push_back(laik2);
		}
	}
	//temp2.push_back(laik2);
